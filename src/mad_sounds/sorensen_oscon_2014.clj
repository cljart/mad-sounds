(ns mad-sounds.sorensen-oscon-2014
  (:use [overtone.live]
...
(defn play
  ...
  (let [start-beat (metro beat)
        end-beat   (metro (+ beat dur))
        cur-synth  (at start-beat (synth :note pitch :level level))]
    (at end-beat (ctl cur-synth :gate 0))))
